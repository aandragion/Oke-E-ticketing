...
public class ad_kursi extends RecyclerView.Adapter<ad_kursi.MyViewHolder> {
    ...
    @NonNull
    @Override
    public ad_kursi.MyViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_kursi, parent, false);
        return new MyViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull final ad_kursi.MyViewHolder holder, final int i) {
        list_kursi model=daftarkursi.get(i);

        if(model!=null){
            holder.kursi.setText(model.getKursi());
            holder.position=i;
